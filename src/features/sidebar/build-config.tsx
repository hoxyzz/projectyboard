...
export function buildSidebarConfig(params: SidebarConfigParams): SidebarConfig {
  ...
  return {
    ...
    teams: teams.map((team) => ({
      ...
      color: team.color,
      items: [
        { id: `${team.id}-issues`, label: "Issues", icon: CircleDot, href: `/${team.id}/issues` },
        { id: `${team.id}-projects`, label: "Projects", icon: FolderKanban, href: `/${team.id}/projects` },
        { id: `${team.id}-views`, label: "Views", icon: Eye, href: `/${team.id}/views` },
      ],
      contextMenu: {
        items: [
          { id: "team-settings", label: "Team settings", icon: Settings, action: () => onTeamSettings?.(team.id) },
          ...
          { id: "leave", label: "Leave team", icon: LogOut, action: () => onLeaveTeam?.(team.id) },
        ],
      },
    })),
