...
export const PriorityIcon = ({ priority, className }: { priority: Priority; className?: string }) => {
...
export const StatusIcon = ({ status, className }: { status: IssueStatus; className?: string }) => {
  switch (status) {
    case "in_progress":
      return (
        <div className={cn("h-3.5 w-3.5 rounded-full border-2 border-li-status-progress border-t-transparent animate-spin", className)} />
      );
    case "done":
      return <Circle className={cn("h-3.5 w-3.5 text-li-status-done fill-li-status-done", className)} />;
    case "cancelled":
      return <Circle className={cn("h-3.5 w-3.5 text-li-dot-red fill-li-dot-red", className)} />;
    default:
  ...
  const description = (() => {
    switch (event.type) {
      case "status_change":
        return (
          <>
            changed status from <span className="text-li-text-bright">{event.from}</span> to{" "}
      ...
      case "priority_change":
        return (
          <>
            changed priority from <span className="text-li-text-bright">{event.from}</span> to{" "}
            <span className="text-li-text-bright">{event.to}</span>
          </>
        );
      case "label_added":
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: "height 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)",
      }}
          ...
          <div className="w-[180px] shrink-0 space-y-3 border-l border-li-divider pl-4">
                ...
                <DropdownMenuContent className="bg-li-menu-bg border-li-menu-border min-w-[140px]">
                  {PRIORITY_OPTIONS.map((opt) => (
                    <DropdownMenuCheckboxItem
                      ...
                      onCheckedChange={() => onPriorityChange(issue, opt.value)}
                      className="text-[12px] text-li-text-bright hover:bg-li-menu-bg-hover cursor-pointer"
                    >
                      {opt.label}
                    </DropdownMenuCheckboxItem>
            ...
            {issue.projectName && (
              <div>
                ...
                <span className="text-[12px] text-li-text">{issue.projectName}</span>
              </div>
            )}
