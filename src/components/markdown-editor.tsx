...
export function MarkdownEditor({
  ...
  return (
    <div className={cn("border border-li-border rounded-md overflow-hidden bg-li-bg", className)}>
      {/* Tabs */}
        ...
        <div className="p-3 min-h-[calc(1.6em*6+1.5rem)] prose-invert max-w-none">
          {value ? (
            <ReactMarkdown
              components={{
                h1: ({ children }) => (
                  <h1 className="text-[18px] font-bold text-li-text-bright mb-2 mt-0">{children}</h1>
                ),
                h2: ({ children }) => (
                  <h2 className="text-[16px] font-semibold text-li-text-bright mb-1.5 mt-3">{children}</h2>
                ),
                h3: ({ children }) => (
                  <h3 className="text-[14px] font-medium text-li-text-bright mb-1 mt-2">{children}</h3>
                ),
                p: ({ children }) => (
                  <p className="text-[13px] text-li-text leading-relaxed mb-2">{children}</p>
                ),
                ul: ({ children }) => (
                  <ul className="text-[13px] text-li-text list-disc pl-5 mb-2 space-y-0.5">{children}</ul>
                ),
                ol: ({ children }) => (
                  <ol className="text-[13px] text-li-text list-decimal pl-5 mb-2 space-y-0.5">{children}</ol>
                ),
                li: ({ children }) => (
                  <li className="text-[13px] text-li-text leading-relaxed">{children}</li>
                ),
                ...
                pre: ({ children }) => (
                  <pre className="bg-li-bg-hover rounded-md overflow-hidden mb-2">{children}</pre>
                ),
                blockquote: ({ children }) => (
                ...
                a: ({ children, href }) => (
                  <a href={href} className="text-li-dot-blue hover:underline" target="_blank" rel="noopener noreferrer">
                    {children}
                  </a>
                ),
                strong: ({ children }) => (
  ...
  return (
    <div className={cn("max-w-none", className)}>
      <ReactMarkdown
        components={{
          h1: ({ children }) => (
            <h1 className="text-[16px] font-bold text-li-text-bright mb-1.5 mt-0">{children}</h1>
          ),
          h2: ({ children }) => (
            <h2 className="text-[14px] font-semibold text-li-text-bright mb-1 mt-2">{children}</h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-[13px] font-medium text-li-text-bright mb-1 mt-1.5">{children}</h3>
          ),
          p: ({ children }) => (
            <p className="text-[12.5px] text-li-text leading-relaxed mb-1.5">{children}</p>
          ),
          ul: ({ children }) => (
            <ul className="text-[12.5px] text-li-text list-disc pl-4 mb-1.5 space-y-0.5">{children}</ul>
          ),
          ol: ({ children }) => (
            <ol className="text-[12.5px] text-li-text list-decimal pl-4 mb-1.5 space-y-0.5">{children}</ol>
          ),
          code: ({ children, className: codeClassName }) => {
            const isInline = !codeClassName;
            return isInline ? (
          ...
          pre: ({ children }) => (
            <pre className="bg-li-bg-hover rounded overflow-hidden mb-1.5">{children}</pre>
          ),
          blockquote: ({ children }) => (
