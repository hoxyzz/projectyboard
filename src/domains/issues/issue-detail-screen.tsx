		...
		switch (event.type) {
			case 'status_change':
				return (
					<>
						changed status from{' '}
						<span className="text-li-text-bright font-medium">{event.from}</span> to{' '}
			...
			case 'label_removed':
				return (
					<>
						removed label{' '}
						<span className="text-li-text-bright font-medium">{event.from}</span>
					</>
				)
			case 'created':
			...
			default:
				return (
					<>
						updated{' '}
						<span className="text-li-text-bright font-medium">{event.field}</span>
					</>
				)
		}
	})()
						...
						<div className="flex-1 min-w-0 space-y-6">
								...
								{editingDesc ? (
									...
									>
										{issue.description ? (
											...
											<span className="text-[13px] text-li-text-muted italic">
							...
							<div>
								...
								{sortedActivity.length > 0 ? (
									...
									<p className="text-[12px] text-li-text-muted italic">
										No activity yet
									</p>
								)}
							</div>
						...
						<div className="w-[220px] shrink-0 space-y-5">
										...
										{PRIORITY_OPTIONS.map((opt) => (
											<DropdownMenuCheckboxItem
												...
												onCheckedChange={() =>
													updateIssue.mutate({
														id: issue.id,
														input: { priority: opt.value as Priority }
													})
												}
												className="text-[12px] text-li-text-bright hover:bg-li-menu-bg-hover cursor-pointer"
											>
							...
							{issue.projectName && (
								<PropertyBlock label="Project">
									<span className="text-[12.5px] text-li-text px-2 -ml-2">
										{issue.projectName}
									</span>
								</PropertyBlock>
							)}
